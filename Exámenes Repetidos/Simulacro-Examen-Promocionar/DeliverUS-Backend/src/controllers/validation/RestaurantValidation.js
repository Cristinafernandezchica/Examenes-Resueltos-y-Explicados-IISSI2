...
// solution: hay algún restaurnate promocionado?  -->  Solo para el método create
const checkHayRestaurantePromocionado = async (ownerId, promotedValue) => {
  if (promotedValue) {
    try {
...
const create = [
  ...
  check('promoted')
    .custom(async (value, { req }) => {
      return checkHayRestaurantePromocionado(req.user.id, value)
    }),
  check('promoted').exists().isBoolean().toBoolean()
  // check('promoted').custom(checkHayRestaurantePromocionado)
]
const update = [
  check('name').exists().isString().isLength({ min: 1, max: 255 }).trim(),
  check('description').optional({ nullable: true, checkFalsy: true }).isString().trim(),
