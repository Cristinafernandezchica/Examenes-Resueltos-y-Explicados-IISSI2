...
export default function EditRestaurantScreen ({ navigation, route }) {
  ...
  // solution: para poder poner el modal de confirmación
  const [percentageConfirmation, setPercentageConfirmation] = useState(null)
  ...
  const updateRestaurant = async (values) => {
    setBackendErrors([])
    // solution: para que el cuadro de confirmación aparezca cuando debe
    if (values.percentage !== 0 && !percentageConfirmation) {
      setPercentageConfirmation(true)
    } else {
      setPercentageConfirmation(false)
      try {
    ...
    <Formik
      ...
      {({ handleSubmit, setFieldValue, values }) => (
        <ScrollView>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
                ...
                <TextSemiBold textStyle={[{ justifyContent: 'space-around', alignSelf: 'center' }]}>
                  Porcentaje actual: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>{values.percentage}%</TextSemiBold>
                </TextSemiBold>
          ...
          {/* solution: ventana de confirmación */}
          <ConfirmationModal
            isVisible={percentageConfirmation !== null}
            onCancel={() => setPercentageConfirmation(null)}
