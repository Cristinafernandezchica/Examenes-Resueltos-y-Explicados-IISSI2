  ...
  const renderProduct = ({ item }) => {
    return (
      ...
      >
        ...
        {/* solution: textos necesarios para ver los nuevos datos */}
        <TextSemiBold>Nutritional compositions:</TextSemiBold>
          <TextSemiBold>  Fats: {item.grasas}</TextSemiBold>
          <TextSemiBold>  Proteins: {item.proteinas}</TextSemiBold>
          <TextSemiBold>  Carbohydrates: {item.carbohidratos}</TextSemiBold>
          <TextSemiBold>  Calories: {item.calorias}</TextSemiBold>
        {!item.availability &&
...
const styles = StyleSheet.create({
  ...
  actionButtonsContainer: {
    ...
    position: 'absolute',
    width: '90%',
    // solution: para desplazar los botones hacia la derecha y que no pisen el texto
    marginLeft: 250
  }
})
