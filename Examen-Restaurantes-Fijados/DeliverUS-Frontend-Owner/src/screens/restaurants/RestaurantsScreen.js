...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        {/* SOLUTION: Añadimos View para que estén ambos elementos en la misma línea */}
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <TextSemiBold>Shipping: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>{item.shippingCosts.toFixed(2)}€</TextSemiBold></TextSemiBold>

          {/* SOLUTION: Botón para fijar/desfijar */}
          <Pressable
  ...
  const pinRestaurant = async (restaurant) => {
    try {
      ...
      showMessage({
        message: `Restaurant ${restaurant.name} succesfully ${restaurant.pinned ? 'unpinned' : 'pinned'}`,
        type: 'success',
      ...
      setRestaurantToBePinned(null)
      showMessage({
        message: `Restaurant ${restaurant.name} could not be ${restaurant.pinned ? 'unpinned' : 'pinned'}.`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
  ...
  return (
    ...
    <ConfirmationModal
      ...
      <TextRegular>You are going to pin/unpin this restaurant. Are you sure?</TextRegular>
    </ConfirmationModal>
    </>
  )
}
