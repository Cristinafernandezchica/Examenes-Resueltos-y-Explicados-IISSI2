...
// SOLUTION: Porcentaje entre -5 y 5 --> No está en uso
/*
const checkLimitPercentage = async (value, { req }) => {
  try {
    const percentage = req.params.percentage
    if (percentage >= 5 || percentage <= -5) {
      return Promise.reject(new Error('The percentage can\'t be out of the range (-5, 5)'))
    } else {
...
const update = [
  ...
  // check('percentage').custom(checkLimitPercentage),
  check('email').optional({ nullable: true, checkFalsy: true }).isString().isEmail().trim(),
  check('phone').optional({ nullable: true, checkFalsy: true }).isString().isLength({ min: 1, max: 255 }).trim(),
  check('restaurantCategoryId').exists({ checkNull: true }).isInt({ min: 1 }).toInt(),
  check('userId').not().exists(),
