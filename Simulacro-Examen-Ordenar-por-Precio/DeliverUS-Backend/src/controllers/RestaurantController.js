...
// SOLUTION: Método para cambiar el estado de sortrByPrice según pulsemos el botón correspodiente
const changeSort = async function (req, res) {
  try {
    const restaurant = await Restaurant.findByPk(req.params.restaurantId)
    restaurant.sortByPrice = !restaurant.sortByPrice
    await restaurant.save() // hay que guardarlo si no, no se hace efectivo el cambio
    res.json(restaurant)
  } catch (err) {
    res.status(500).send(err)
  }
